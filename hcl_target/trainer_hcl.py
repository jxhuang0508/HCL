...
import copy
import numpy as np
from utils.loss import Contrast_loss_his
#fp16
try:
    import apex
...
class AD_Trainer(nn.Module):
    ...
    def update_consistency_his(self, labels, pred1, pred2, pred1_ma, pred2_ma, pred1_source, pred2_source):
            criterion = nn.CrossEntropyLoss(weight = self.class_weight, ignore_index=255, reduction = 'none')
            kl_distance = nn.KLDivLoss( reduction = 'none')
            loss = criterion(pred1, labels)
    ...
    def update_contrast_his(self, label_label_aug1, feature, labels, feature_ma, pred1_ma):
            ...
            labels = (F.interpolate(labels.clone().type(torch.FloatTensor).unsqueeze(1), size=feature.shape[2:4], mode='nearest')).view(labels.shape[0], -1)
    ...
    def gen_update(self, images, images_t, labels, labels_t, i_iter, image_aug1, label_label_aug1):
            ...
            with torch.no_grad():  # no gradient to keys
                pred1_ma, pred2_ma, feature_1_ma = self.G_ma(images)
                pred1_ma = self.interp(pred1_ma)
                pred2_ma = self.interp(pred2_ma)
                pred1_source, pred2_source, _ = self.G_source(images)
                pred1_source = self.interp(pred1_source)
                pred2_source = self.interp(pred2_source)
            # pred1, pred2 = self.G(images)
            pred1, pred2, feature_1 = self.G(image_aug1)
            ...
            loss_seg1 = self.update_consistency_his(label_label_aug1, pred1, pred2, pred1_ma, pred2_ma, pred1_source, pred2_source)
            ...
            # target
            pred_target1, pred_target2, _ = self.G(images_t)
            ...
            if i_iter % 300 == 0 and i_iter != 0:
                ...
                for param_q, param_k, param_source in zip(self.G.parameters(), self.G_ma.parameters(), self.G_source.parameters()):
                    ...
                    param_source.data = param_q.data.clone()
                for buffer_q, buffer_k, buffer_source in zip(self.G.buffers(), self.G_ma.buffers(), self.G_source.buffers()):
                    buffer_k.data = buffer_q.data.clone()
                    buffer_source.data = buffer_q.data.clone()
